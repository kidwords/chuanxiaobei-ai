...
const openSourceProjects = [
  {
    ...
    description:
      "整理《中国传统色：故宫里的色彩美学》完整 384 色，按 24 节气与 72 物候编排，附标准 Hex 色值与 AI 配色使用建议。",
...
function TraditionalColorArtwork({
  image,
  title,
}: Pick<(typeof openSourceProjects)[number], "image" | "title">) {
  return (
...
function ArrowUpRightIcon() {
  return <ArrowRight aria-hidden="true" focusable="false" className="-rotate-45" size={16} />;
}
          ...
          <div className="mx-auto flex h-20 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <a className="hidden h-10 items-center gap-2 rounded-[10px] bg-zinc-950 px-4 text-sm font-medium text-white shadow-sm transition-all duration-250 hover:-translate-y-0.5 hover:bg-zinc-800 hover:shadow-md sm:inline-flex dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-200" href="#contact">
                开始交流 <ArrowRight aria-hidden="true" focusable="false" size={15} />
              </a>
              <MobileMenu>
                <NavLink href="#projects">项目</NavLink>
                <NavLink href="#about">关于</NavLink>
          ...
          <section className="mx-auto grid max-w-6xl gap-12 px-4 pb-20 pt-20 sm:px-6 lg:grid-cols-[1fr_320px] lg:items-end lg:pb-28 lg:pt-28">
            <div>
              <div className="inline-flex items-center gap-2 rounded-[10px] border border-zinc-200 bg-white px-3 py-2 text-sm font-medium text-zinc-600 shadow-sm dark:border-white/10 dark:bg-white/5 dark:text-zinc-300">
                <Sparkles aria-hidden="true" focusable="false" size={15} className="text-blue-600 dark:text-blue-400" /> 独立开发 · AI 实践 · 产品设计
              </div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <a className="inline-flex h-11 items-center gap-2 rounded-[10px] bg-blue-600 px-5 text-sm font-medium text-white shadow-sm shadow-blue-600/20 transition-all duration-250 hover:-translate-y-0.5 hover:bg-blue-700 hover:shadow-md" href="#projects">
                  浏览项目 <ArrowDownRight aria-hidden="true" focusable="false" size={16} />
                </a>
                <a className="inline-flex h-11 items-center gap-2 rounded-[10px] border border-zinc-200 bg-white px-5 text-sm font-medium text-zinc-800 shadow-sm transition-all duration-250 hover:-translate-y-0.5 hover:border-zinc-300 hover:shadow-md dark:border-white/10 dark:bg-white/5 dark:text-white" href="#about">
                  了解我 <ArrowRight aria-hidden="true" focusable="false" size={16} />
                </a>
              </div>
            </div>
                    ...
                    <div className="flex flex-col p-6 sm:p-8">
                      ...
                      <div className="mt-6 flex flex-wrap gap-2">
                        {project.tags.map((tag) => (
                          <span className="rounded-[10px] bg-zinc-100 px-2.5 py-1 text-xs font-medium text-zinc-600 dark:bg-white/10 dark:text-zinc-300" key={tag}>{tag}</span>
                        ))}
                      </div>
                      <div className="mt-8 flex flex-wrap gap-3">
                        <a className="inline-flex h-10 items-center gap-2 rounded-[10px] bg-zinc-950 px-4 text-sm font-medium text-white transition-all duration-250 hover:-translate-y-0.5 hover:bg-zinc-800 dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-200" href={project.github} target="_blank" rel="noreferrer">
                          <GitFork aria-hidden="true" focusable="false" size={16} /> 查看源代码 <ExternalLink aria-hidden="true" focusable="false" size={14} />
                        </a>
            ...
            <div>
              ...
              <div className="mt-8 grid gap-3">
                <a className="group flex items-center justify-between rounded-[10px] border border-zinc-200 bg-white px-4 py-3 shadow-sm transition-all duration-250 hover:-translate-y-0.5 hover:border-zinc-300 hover:shadow-md dark:border-white/10 dark:bg-zinc-900 dark:hover:border-white/20" href="https://github.com/kidwords" target="_blank" rel="noreferrer">
                  ...
                  <ExternalLink aria-hidden="true" focusable="false" className="text-zinc-400 transition-transform duration-250 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 dark:text-zinc-300" size={16} />
                </a>
            ...
            <div className="grid gap-4">
              {capabilities.map(([title, description, Icon], index) => (
                <div className="flex gap-4 rounded-[14px] border border-zinc-200 bg-white p-5 shadow-sm transition-all duration-250 hover:-translate-y-0.5 hover:shadow-md dark:border-white/10 dark:bg-zinc-900" key={title}>
                  ...
                  <div>
                    <span className="text-xs font-medium text-zinc-500 dark:text-zinc-400">0{index + 1}</span>
                    <h3 className="mt-1 font-semibold">{title}</h3>
                    <p className="mt-2 text-sm leading-[1.7] text-zinc-600 dark:text-zinc-300">{description}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section id="contact" className="bg-zinc-950 px-4 py-16 text-white sm:px-6 sm:py-20">
            <div className="mx-auto flex max-w-6xl flex-col justify-between gap-8 lg:flex-row lg:items-end">
              ...
              <div className="flex flex-wrap gap-3">
                <a className="inline-flex h-11 items-center gap-2 rounded-[10px] bg-white px-5 text-sm font-medium text-zinc-950 transition-all duration-250 hover:-translate-y-0.5 hover:bg-zinc-200" href="https://github.com/kidwords" target="_blank" rel="noreferrer">
                  <GitFork aria-hidden="true" focusable="false" size={16} /> GitHub 主页 <ExternalLink aria-hidden="true" focusable="false" size={14} />
                </a>
              </div>
            </div>
          </section>
        </main>
        <footer className="border-t border-white/10 bg-zinc-950 px-4 py-6 text-sm text-zinc-500 sm:px-6">
          <div className="mx-auto flex max-w-6xl flex-col justify-between gap-2 sm:flex-row">
            <span>© 2026 川小北AI</span>
