...
- **订阅节气 / 收藏配色**：在本机记住节气订阅与自选配色，刷新后仍在。没有付费墙，也不会发邮件。
...
8. 点击“订阅节气”，首页会出现「今日节气」摘要，并可跳到该节气的 16 色。
9. 打开“配色板”，点选颜色后点击“收藏此配色”；“我的收藏”可还原或删除。数据写在浏览器 localStorage 键 `ctc-subscription`。

## 订阅与收藏（本机）

这一版是纯前端切片，不需要后端。

- 存储键：`ctc-subscription`（localStorage）
- 订阅节气：首页或沉浸页点击「订阅节气」。订阅后首页显示「今日节气」摘要（节气名、日期、下一节气），以及跳到该节气 16 色的入口。会记住上次看到的节气；若已更替，会出现横幅。浏览器通知是可选的，拒绝后界面照常工作。
- 收藏配色：在「配色板」点选颜色后点「收藏此配色」。名称与 Hex 列表保存在本机。「我的收藏」可还原或删除。
- 邮箱栏是本机候补名单 / 提醒，**不会发信**。邮件推送是后续步骤。
- 若浏览器禁用 localStorage，功能仍可在本次打开期间使用，并会提示刷新后会丢失。
- 从仓库首页可通过 `#subscribe` 打开订阅面板：`chinese-traditional-colors/index.html#subscribe`
...
- 点击首页单色、混色、逐步览色、全部浏览、订阅节气和配色板入口。
- 订阅后刷新页面，确认「今日节气」仍在；在配色板收藏一组颜色，刷新后再打开「我的收藏」确认仍在。
